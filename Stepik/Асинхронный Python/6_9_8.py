...
data = [
    ...
    {'Name': 'Company11', 'Address': '4709 Brooks Camp Suite 799, Port Natalie, NY 99637', 'Phone': '(818)703-8797',
     ...
     'call_time': 1},
    {'Name': 'Company12', 'Address': '0372 Jared Isle Suite 075, South Anthony, GU 21805',
...
async def call_company(d):
    try:

        await asyncio.sleep(d['call_time'])
        if d['call_time'] > 5:
            raise asyncio.CancelledError()
        else:
            print(f'Company {d["Name"]}: {d["Phone"]} дозвон успешен')
    except asyncio.CancelledError:
        pass

async def main():
    tasks = [asyncio.create_task(call_company(i)) for i in data]
